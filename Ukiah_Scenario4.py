...
model_name = 'Flopy_Output_Scenario4\\Ukiah'
...
if Offlineextentflag:
    ...
    active1 = np.loadtxt('Input_Data\\ASCI_Files_June\\Layer_4_Res3.asc', skiprows =6) # Where to assign the active layer for the RESERVOIR
    active2 = np.loadtxt('Input_Data\\ASCI_Files_June\\Active_Layer2_OnlyAct.asc', skiprows =0)
    active3 = np.loadtxt('Input_Data\\ASCI_Files_June\\Active_Layer3_OnlyAct.asc',skiprows =0)
    active4 = np.loadtxt('Input_Data\\ASCI_Files_June\\Active_Layer4_All.asc', skiprows =0)
...
#### 1965 Hist
segment_data = pd.read_csv('Input_Data\\SFR_New\\With_Diversion\\SI_1991-2018_Scenario4.csv')
...
for i in range(strt_rech,end_rech):
    rech_dict[i] = np.loadtxt('Input_Data\\Recharge_MAR\\Scenario4\\Rech_MAR_'+str(i+312)+'.asc')
...
obs = flopy.modflow.ModflowHob.load('Input_Data\\Observations\\Ukiah_June_fast_1991.hob', mf)
